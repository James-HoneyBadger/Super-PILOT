"""Benchmark each Time Warp command by calling interpreter command handlers directly.
...
import contextlib
import io
from pilot import TimeWarpInterpreter
...
def bench_case(interp, name, cmd, repeats, kind):
    ...
    print(
        f"{name:10s} | repeats={repeats:6d} | total={total:.6f}s | avg={avg_us:.2f}us"
    )
...
def main():
    print("Time Warp command benchmark — running quick microbenchmarks")
